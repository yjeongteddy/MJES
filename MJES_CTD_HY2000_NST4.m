...
for i = 1:n_sz
    lat_mcllcted_nst4(i) = nanmean(lat_cllcted(:,i));
    lon_mcllcted_nst4(i) = nanmean(lon_cllcted(:,i));
end
for i = 1:n_sz
    prs_length(i) = length(prs{1,i});
end
idx_prs = find(prs_length == max(prs_length));
[mlon_nst4 mprs_nst4] = meshgrid(lon_mcllcted_nst4,prs{1,idx_prs}(:,1)); %arbitrary chosen

%% Bottom topography
lon_for_bathy_nst4 = linspace(min(lon_mcllcted_nst4),max(lon_mcllcted_nst4),100);
lat_for_bathy_nst4 = linspace(max(lat_mcllcted_nst4),min(lat_mcllcted_nst4),100);

for i = 1:length(lon_for_bathy_nst4)
    [BATHY_nst4(i),XGRID_nst4(i),YGRID_nst4(i)] = gebconetcdf('C:\matlib\gebco_2021\gebco_08.nc',lon_for_bathy_nst4(i),lon_for_bathy_nst4(i),lat_for_bathy_nst4(i),lat_for_bathy_nst4(i));
end
...
ax(1) = subplot(2,2,1);
pcolor(mlon_nst4,mprs_nst4,TEMP_cllcted(mprs_nst4(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
[c1 h1] = contour(mlon_nst4,mprs_nst4,TEMP_cllcted(mprs_nst4(1,1):end,:),[0:0.1:1 5:1.5:20],'-w','LabelSpacing',1000);
...
ax(2) = subplot(2,2,2);
pcolor(mlon_nst4,mprs_nst4,sal_cllcted(mprs_nst4(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([34.0 34.5]);
[c1 h1] = contour(mlon_nst4,mprs_nst4,sal_cllcted(mprs_nst4(1,1):end,:),[34:0.1:34.5],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst4,'-r',{'NST4-2','NST4-3','NST4-4','NST4-5'},'LineWidth',.5,'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst4) max(lon_mcllcted_nst4)]);
...
ax(3) = subplot(2,2,3);
pcolor(mlon_nst4,mprs_nst4,dnsty_cllcted(mprs_nst4(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([24.5 27.5]);
[c1 h1] = contour(mlon_nst4,mprs_nst4,dnsty_cllcted(mprs_nst4(1,1):end,:),[24.5:0.25:27.5],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst4,'-r',{'NST4-2','NST4-3','NST4-4','NST4-5'},'LineWidth',.5,'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst4) max(lon_mcllcted_nst4)]);
...
ax(4) = subplot(2,2,4);
pcolor(mlon_nst4,mprs_nst4,fl_cllcted(mprs_nst4(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([0 0.9]);
[c1 h1] = contour(mlon_nst4,mprs_nst4,fl_cllcted(mprs_nst4(1,1):end,:),[0.2 0.3],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst4,'-r',{'NST4-2','NST4-3','NST4-4','NST4-5'},'LineWidth',.5,'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst4) max(lon_mcllcted_nst4)]);
...
for i = 1:length(files)
    n = n+1;
    ds_NST4_CTD_target{n} = importdata(files(i).name);
end

%% Get maximum size of row of each column
for i = 1:length(files)
    num_of_row_max_NST4(i) = max(size(ds_NST4_CTD_target{1,i}));
end

%% Assign pressure
for i = 1:length(files)
    prs_NST4{i} = ds_NST4_CTD_target{1,i}.data(:,2);
end

%% Creat nan matrix for each variable
prs_new_NST4 = nan(max(num_of_row_max_NST4),length(files));
potemp_new_NST4 = nan(max(num_of_row_max_NST4),length(files));
psal_new_NST4 = nan(max(num_of_row_max_NST4),length(files));

%% Assign rest variables
for i = 1:length(files)
    prs_new_NST4(prs_NST4{1,i},i) = ds_NST4_CTD_target{1,i}.data(:,2);
    potemp_new_NST4(prs_NST4{1,i},i) = ds_NST4_CTD_target{1,i}.data(:,11);
    psal_new_NST4(prs_NST4{1,i},i) = ds_NST4_CTD_target{1,i}.data(:,5);
end

%% Slice some rows to make a consistent form of matrix(otherwise errors hit)
for i = 1:length(files)
    ga_NST4(3:96,i) = sw_gpan(psal_new_NST4(3:96,i),potemp_new_NST4(3:96,i),prs_new_NST4(3:96,i));
end

%% Assign lat, lon from VS, VP CTD data
for i = 1:length(files)
    lat_NST4{i} = ds_NST4_CTD_target{1,i}.data(19,6);
    lon_NST4{i} = ds_NST4_CTD_target{1,i}.data(19,7);
end

%% Convert them into a double form
lat_NST4 = cell2mat(lat_NST4);
lon_NST4 = cell2mat(lon_NST4);

%% Calculate geostrophic velocity
gvel_NST4 = sw_gvel(ga_NST4,lat_NST4,lon_NST4);

%% Level of no motion (lnm)
[m_gvel_NST4 n_gvel_NST4] = size(gvel_NST4);
for i = 1:n_gvel_NST4
    if gvel_NST4(end,i) < 0
        gvel_NST4(4:end,i) = gvel_NST4(4:end,i) + abs(gvel_NST4(m_gvel_NST4,i));
    elseif gvel_NST4(end,i) > 0
        gvel_NST4(4:end,i) = gvel_NST4(4:end,i) - abs(gvel_NST4(m_gvel_NST4,i));
    end
end

%% Find middle points of each lon
for i = 1:length(lon_NST4)-1
    lon_mean_NST4(i) = (lon_NST4(i)+lon_NST4(i+1))./2;
end

%% Define new meshgrids
[mlon_NST4 mdepth_NST4] = meshgrid(lon_mean_NST4,prs_new_NST4(3:96,2));

%% Plot
figure;set(gcf,'position',[403 42 1149 954]);
ax(4) = subplot(2,2,4);
pcolor(mlon_NST4,mdepth_NST4,gvel_NST4(3:end,:));shading interp;hold on;
CT = flip(cbrewer('div','RdBu',50,'linear'),1);
colormap(CT);
set(gca,'YDir','reverse');
caxis([-0.7 0.7001]);
[c1 h1] = contour(mlon_NST4,mdepth_NST4,gvel_NST4(3:end,:),[-0.7:0.1:0.7],'-k','LabelSpacing',1000,'LineWidth',1.5);
