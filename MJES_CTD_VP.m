...
[mlon mprs] = meshgrid(lon_mcllcted,prs{1,idx_prs}(:,1)); %arbitrary chosen

%% Bottom topography
lon_for_bathy = linspace(min(lon_mcllcted),max(lon_mcllcted),100);
lat_for_bathy = linspace(max(lat_mcllcted),min(lat_mcllcted),100);
...
%% Buoyancy Frequency
[SA_VP, in_ocean_VB] = gsw_SA_from_SP(sal_cllcted,prs_cllcted,lon_cllcted,lat_cllcted);
...
for i = 1:length(files)
    n = n+1;
    ds_VP_CTD_target{n} = importdata(files(i).name);
end
...
%% Assign pressure
for i = 1:length(files)
    prs_VP{i} = ds_VP_CTD_target{1,i}.data(:,2);
end

%% Creat nan matrix for each variable
prs_new_VP = nan(max(num_of_row_max_VP),length(files));
potemp_new_VP = nan(max(num_of_row_max_VP),length(files));
psal_new_VP = nan(max(num_of_row_max_VP),length(files));

%% Assign rest variables
for i = 1:length(files)
    prs_new_VP(prs_VP{1,i},i) = ds_VP_CTD_target{1,i}.data(:,2);
    potemp_new_VP(prs_VP{1,i},i) = ds_VP_CTD_target{1,i}.data(:,11);
    psal_new_VP(prs_VP{1,i},i) = ds_VP_CTD_target{1,i}.data(:,5);
end

%% Slice some rows to make a consistent form of matrix(otherwise errors hit)
for i = 1:length(files)
    ga_VP(4:127,i) = sw_gpan(psal_new_VP(4:127,i),potemp_new_VP(4:127,i),prs_new_VP(4:127,i));
end

%% Assign lat, lon from VS, VP CTD data
for i = 1:length(files)
    lat_VP{i} = ds_VP_CTD_target{1,i}.data(19,6);
    lon_VP{i} = ds_VP_CTD_target{1,i}.data(19,7);
end
...
gvel_VP = sw_gvel(ga_VP,lat_mcllcted,lon_mcllcted);

%% Level of no motion (lnm)
[m_gvel_VP n_gvel_VP] = size(gvel_VP);
for i = 1:n_gvel_VP
    if gvel_VP(end,i) < 0
        gvel_VP(4:end,i) = gvel_VP(4:end,i) + abs(gvel_VP(m_gvel_VP,i));
    elseif gvel_VP(end,i) > 0
        gvel_VP(4:end,i) = gvel_VP(4:end,i) - abs(gvel_VP(m_gvel_VP,i));
    end
end

%% Find middle points of each lon
for i = 1:length(lon_mcllcted)-1
    lon_mean_mcllcted(i) = (lon_mcllcted(i)+lon_mcllcted(i+1))./2;
end

%% Define new meshgrids
[mlon_VP_geo mdepth_VP_geo] = meshgrid(lon_mean_mcllcted,prs_new_VP(4:127,2));
...
ax(1) = subplot(2,3,1);
pcolor(mlon,mprs,TEMP_cllcted(mprs(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
[c1 h1] = contour(mlon,mprs,TEMP_cllcted(mprs(1,1):end,:),[0:0.1:1 5:1.5:20],'-w','LabelSpacing',1000,'LineWidth',1);
clabel(c1,h1,'color','w','FontSize',12,'FontWeight','b');
xline(lon_mcllcted,'-k',{'VP3','VP7','VP11','VP15','VP19'},'LineWidth',.5, ...
    'LabelVerticalAlignment','bottom','LabelHorizontalAlignment','center','FontWeight','b','FontName','Times New Roman','FontSize',22);
xlim([min(lon_mcllcted)-0.001 max(lon_mcllcted)]);
ylim([0 max(mprs(:,1))]);
xtickformat('%.2f');
...
xlabel('Longitude (^oN)','Fontname','Times New Roman','FontWeight','b','FontSize',30);
ylabel('Pressure (db)','Fontname','Times New Roman','FontWeight','b','FontSize',30);
area(lon_for_bathy,-BATHY,max(mprs(:,1)),'Facecolor',[0.7 0.7 0.7],'EdgeColor','k','LineWidth',1.5);
hold off;

ax(2) = subplot(2,3,2);
pcolor(mlon,mprs,sal_cllcted(mprs(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
[c1 h1] = contour(mlon,mprs,sal_cllcted(mprs(1,1):end,:),[34:0.1:34.5],'-w','LabelSpacing',1000,'LineWidth',1);
clabel(c1,h1,'color','w','FontSize',12,'FontWeight','b');
...
ax(3) = subplot(2,3,3);
pcolor(mlon,mprs,dnsty_cllcted(mprs(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([24.5 27.5]);
[c1 h1] = contour(mlon,mprs,dnsty_cllcted(mprs(1,1):end,:),[24.5:0.5:27.5],'-w','LabelSpacing',1000,'LineWidth',1);
clabel(c1,h1,'color','w','FontSize',12,'FontWeight','b');
xline(lon_mcllcted,'-r',{'VP3','VP7','VP11','VP15','VP19'},'LineWidth',.5, ...
    'LabelVerticalAlignment','bottom','LabelHorizontalAlignment','center','FontWeight','b','FontName','Times New Roman','FontSize',22);
xlim([min(lon_mcllcted)-0.001 max(lon_mcllcted)]);
...
figure;hold on;set(gcf,'position',[1 41 1920 963]);
ax(4) = subplot(2,3,4);
pcolor(mlon,p_mid_VP(2:end,:),N2_VP(2:end,:));shading interp;hold on;
CT = cbrewer('seq','Reds',50,'linear');
colormap(CT);
set(gca,'YDir','reverse');
caxis([0 0.0006]);
[c1 h1] = contour(mlon,mprs,dnsty_cllcted(mprs(1,1):end,:),[24.5:0.25:27.5],'-k','LabelSpacing',1000,'LineWidth',1.5);
...
figure;hold on;set(gcf,'position',[1 41 1920 963]);
ax(5) = subplot(2,3,5);
pcolor(mlon_VP_geo,mdepth_VP_geo,gvel_VP(4:end,:));shading interp;hold on;
CT = flip(cbrewer('div','RdBu',50,'linear'),1);
colormap(CT);
set(gca,'YDir','reverse');
caxis([-0.9 0.9001]);
[c1 h1] = contour(mlon_VP_geo,mdepth_VP_geo,gvel_VP(4:end,:),[-0.7:0.1:0.7],'-k','LineWidth',1.5);
clabel(c1,h1,'color','k','FontSize',10,'FontWeight','b');
xline(lon_mcllcted,'-r',{'VP3','VP7','VP11','VP15','VP19'},'LineWidth',.5, ...
    'LabelVerticalAlignment','bottom','LabelHorizontalAlignment','center','FontWeight','b','FontName','Times New Roman','FontSize',22);
xlim([min(lon_mcllcted)-0.001 max(lon_mcllcted)]);
...
%% For automation
% 1. meshgrid 사용 시 mlon을 쓸건지 mlat을 쓸건지 해역마다 다르기 때문에 해당 부분에 대한 자동화 필요
% 2. xline 그릴 때도 마찬가지로 mlon을 쓰면 lon_mcllcted를 써야하고, mlat을 쓰면 lat_mcllcted를 써야 하는데 이것도 자동화 필요
% 3. xlim을 쓸 때 min() max()를 이용해서 하는데 이 때도 마찬가지로 lat_mcllcted와 lon_mcllcted를 상황에 맞게 달리해서 써야해서 자동화 다시 필요
% 4. xline을 그릴 때 라인 이름에 정점명을 넣는데 이때 해역마다 정점명이 다르므로 이것도 자동화 필요
