...
num_max_row = max(ds_size);prs_cllcted = nan(num_max_row,length(ds_size));TimeJ_cllcted = nan(num_max_row,length(ds_size));
...
for g = 1:length(files)
    ...
    TimeJ_cllcted(prs{g},g) = TimeJ{1,g};
...
TimeJ_cllcted(TimeJ_cllcted == 0) = nan;
...
%% Time Set
[m n] = size(TimeJ_cllcted);
start_time = datenum([2021,11,13,15,0,0]);
for i = 1:n
    Time_cllcted(:,i) = datenum([2021,11,13,15+i-1,0,0]);
end
...
[mTime_KU mprs_KU] = meshgrid(Time_cllcted,prs{1,idx_prs}(:,1)); %arbitrary chosen

%% Buoyancy Frequency
[SA_KU, in_ocean_KU] = gsw_SA_from_SP(sal_cllcted,prs_cllcted,lon_cllcted,lat_cllcted);
CT_KU = gsw_CT_from_pt(SA_KU,TEMP_cllcted);
[N2_KU, p_mid_KU] = gsw_Nsquared(SA_KU,CT_KU,prs_cllcted,lat_cllcted);

%% Plot
% KU yoyo
figure;hold on;set(gcf,'position',[252 42 1271 954]);
ax(1) = subplot(2,1,1);
pcolor(mTime_KU,mprs_KU,TEMP_cllcted(mprs_KU(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([0 22]);
[c1 h1] = contour(mTime_KU,mprs_KU,TEMP_cllcted(mprs_KU(1,1):end,:),[0.1:0.1:1 12:2:22],'-w','LabelSpacing',1000,'LineWidth',1.5);
...
xtickformat('%.2f');
ylim([0 250]);
h = colorbar; h.FontWeight = 'b';h.FontName = 'Times New Roman';h.FontSize = 20;
title(h,'[\circC]','Fontname','Times New Roman','fontweight','b','FontSize',20);
title('Potential temperature','Fontname','Times New Roman','FontWeight','b','FontSize',30);
...
ax(2) = subplot(2,1,2);
pcolor(mTime_KU,mprs_KU,sal_cllcted(mprs_KU(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([34.0 34.5]);
[c1 h1] = contour(mTime_KU,mprs_KU,sal_cllcted(mprs_KU(1,1):end,:),[34:0.1:34.5],'-w','LabelSpacing',1000,'LineWidth',1.5);
clabel(c1,h1,'color','w','FontName','Times New Roman','FontWeight','b','FontSize',15);
xline(Time_cllcted,':k','LineWidth',.5, 'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xtickformat('%.1f');
...
title(h,'[psu]','Fontname','Times New Roman','FontWeight','b','FontSize',20);
title('Practical salinity','Fontname','Times New Roman','FontWeight','b','FontSize',30);
...
figure;hold on;set(gcf,'position',[252 42 1271 954]);
ax(1) = subplot(2,1,1);
pcolor(mTime_KU,mprs_KU,dnsty_cllcted(mprs_KU(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([24.5 27.5]);
[c1 h1] = contour(mTime_KU,mprs_KU,dnsty_cllcted(mprs_KU(1,1):end,:),[24.5:0.5:27.5],'-w','LabelSpacing',1000,'LineWidth',1.5);
clabel(c1,h1,'color','w','FontName','Times New Roman','FontWeight','b','FontSize',15);
xline(Time_cllcted,':k','LineWidth',.5, 'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xtickformat('%.1f');
...
h = colorbar; h.FontWeight = 'b';h.FontName = 'Times New Roman';h.FontSize = 20;
h.Ruler.TickLabelFormat = '%.1f';
title(h,'[kg/m^3]','Fontname','Times New Roman','FontWeight','b','FontSize',20);
title('Potential density','Fontname','Times New Roman','FontWeight','b','FontSize',30);
set(ax(1).XAxis,'Fontname','Times New Roman','FontWeight','b','FontSize',20);
set(ax(1).YAxis,'Fontname','Times New Roman','FontWeight','b','FontSize',20);
datetick('x','hh');
xlim([Time_cllcted(1) Time_cllcted(end)]);
...
ax(2) = subplot(2,1,2);
pcolor(mTime_KU,mprs_KU,N2_KU);shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([0 0.0008]);
[c1 h1] = contour(mTime_KU,mprs_KU,dnsty_cllcted(mprs_KU(1,1):end,:),[24.5:0.5:27.5],'-w','LabelSpacing',1000,'LineWidth',1.5);
...
title('N^2','Fontname','Times New Roman','FontWeight','b','FontSize',30);
set(ax(2).XAxis,'Fontname','Times New Roman','FontWeight','b','FontSize',20);
set(ax(2).YAxis,'Fontname','Times New Roman','FontWeight','b','FontSize',20);
datetick('x','hh');
xlim([Time_cllcted(1) Time_cllcted(end)]);
...
for i = 1:length(files)
    n = n+1;
    ds_NST1_CTD_target{n} = importdata(files(i).name);
end

%% Get maximum size of row of each column
for i = 1:length(files)
    num_of_row_max_NST1(i) = max(size(ds_NST1_CTD_target{1,i}));
end

%% Assign pressure
for i = 1:length(files)
    prs_NST1{i} = ds_NST1_CTD_target{1,i}.data(:,2);
end

%% Creat nan matrix for each variable
prs_new_NST1 = nan(max(num_of_row_max_NST1),length(files));
potemp_new_NST1 = nan(max(num_of_row_max_NST1),length(files));
psal_new_NST1 = nan(max(num_of_row_max_NST1),length(files));

%% Assign rest variables
for i = 1:length(files)
    prs_new_NST1(prs_NST1{1,i},i) = ds_NST1_CTD_target{1,i}.data(:,2);
    potemp_new_NST1(prs_NST1{1,i},i) = ds_NST1_CTD_target{1,i}.data(:,11);
    psal_new_NST1(prs_NST1{1,i},i) = ds_NST1_CTD_target{1,i}.data(:,5);
end

%% Slice some rows to make a consistent form of matrix(otherwise errors hit)
for i = 1:length(files)
    ga_NST1(3:93,i) = sw_gpan(psal_new_NST1(3:93,i),potemp_new_NST1(3:93,i),prs_new_NST1(3:93,i));
end

%% Assign lat, lon from VS, VP CTD data
for i = 1:length(files)
    lat_NST1{i} = ds_NST1_CTD_target{1,i}.data(19,6);
    lon_NST1{i} = ds_NST1_CTD_target{1,i}.data(19,7);
end

%% Convert them into a double form
lat_NST1 = cell2mat(lat_NST1);
lon_NST1 = cell2mat(lon_NST1);

%% Calculate geostrophic velocity
gvel_NST1 = sw_gvel(ga_NST1,lat_NST1,lon_NST1);

%% Level of no motion (lnm)
[m_gvel_NST1 n_gvel_NST1] = size(gvel_NST1);
for i = 1:n_gvel_NST1
    if gvel_NST1(end,i) < 0
        gvel_NST1(4:end,i) = gvel_NST1(4:end,i) + abs(gvel_NST1(m_gvel_NST1,i));
    elseif gvel_NST1(end,i) > 0
        gvel_NST1(4:end,i) = gvel_NST1(4:end,i) - abs(gvel_NST1(m_gvel_NST1,i));
    end
end

%% Find middle points of each lon
for i = 1:length(lon_NST1)-1
    lon_mean_NST1(i) = (lon_NST1(i)+lon_NST1(i+1))./2;
end

%% Define new meshgrids
[mlon_NST1 mdepth_NST1] = meshgrid(lon_mean_NST1,prs_new_NST1(3:93,2));

%% Plot
figure;set(gcf,'position',[403 42 1149 954]);
ax(1) = subplot(2,2,1);
pcolor(mlon_NST1,mdepth_NST1,gvel_NST1(3:end,:));shading interp;hold on;
CT = flip(cbrewer('div','RdBu',50,'linear'),1);
colormap(CT);
set(gca,'YDir','reverse');
caxis([-0.7 0.7001]);
[c1 h1] = contour(mlon_NST1,mdepth_NST1,gvel_NST1(3:end,:),[-0.7:0.1:0.7],'-k','LabelSpacing',1000,'LineWidth',1.5);
...
area(lon_for_bathy,-BATHY,max(mprs_KU(:,1)),'Facecolor',[0.7 0.7 0.7],'EdgeColor','k','LineWidth',1.5);
