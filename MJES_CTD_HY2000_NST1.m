...
for i = 1:n_sz
    lat_mcllcted_nst1(i) = nanmean(lat_cllcted(:,i));
    lon_mcllcted_nst1(i) = nanmean(lon_cllcted(:,i));
end
for i = 1:n_sz
    prs_length(i) = length(prs{1,i});
end
idx_prs = find(prs_length == max(prs_length));
[mlon_nst1 mprs_nst1] = meshgrid(lon_mcllcted_nst1,prs{1,idx_prs}(:,1)); %arbitrary chosen

%% Bottom topography
lon_for_bathy_nst1 = linspace(min(lon_mcllcted_nst1),max(lon_mcllcted_nst1),100);
lat_for_bathy_nst1 = linspace(max(lat_mcllcted_nst1),min(lat_mcllcted_nst1),100);

for i = 1:length(lon_for_bathy_nst1)
    [BATHY_nst1(i),XGRID_nst1(i),YGRID_nst1(i)] = gebconetcdf('C:\matlib\gebco_2021\gebco_08.nc',lon_for_bathy_nst1(i),lon_for_bathy_nst1(i),lat_for_bathy_nst1(i),lat_for_bathy_nst1(i));
end
...
ax(1) = subplot(2,2,1);
pcolor(mlon_nst1,mprs_nst1,TEMP_cllcted(mprs_nst1(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
[c1 h1] = contour(mlon_nst1,mprs_nst1,TEMP_cllcted(mprs_nst1(1,1):end,:),[0:0.1:1 5:1.5:20],'-w','LabelSpacing',1000);
...
ax(2) = subplot(2,2,2);
pcolor(mlon_nst1,mprs_nst1,sal_cllcted(mprs_nst1(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([34.0 34.5]);
[c1 h1] = contour(mlon_nst1,mprs_nst1,sal_cllcted(mprs_nst1(1,1):end,:),[34:0.1:34.5],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst1,'-r',{'NST1-2','NST1-3','NST1-4','NST1-5'},'LineWidth',.5, 'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst1) max(lon_mcllcted_nst1)]);
...
ax(3) = subplot(2,2,3);
pcolor(mlon_nst1,mprs_nst1,dnsty_cllcted(mprs_nst1(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([24.5 27.5]);
[c1 h1] = contour(mlon_nst1,mprs_nst1,dnsty_cllcted(mprs_nst1(1,1):end,:),[24.5:0.5:27.5],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst1,'-r',{'NST1-2','NST1-3','NST1-4','NST1-5'},'LineWidth',.5, 'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst1) max(lon_mcllcted_nst1)]);
...
ax(4) = subplot(2,2,4);
pcolor(mlon_nst1,mprs_nst1,fl_cllcted(mprs_nst1(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([0 0.9]);
[c1 h1] = contour(mlon_nst1,mprs_nst1,fl_cllcted(mprs_nst1(1,1):end,:),[0.2 0.3],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst1,'-r',{'NST1-2','NST1-3','NST1-4','NST1-5'},'LineWidth',.5, 'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst1) max(lon_mcllcted_nst1)]);
