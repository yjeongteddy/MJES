...
for i = 1:n_sz
    lat_mcllcted_nst3(i) = nanmean(lat_cllcted(:,i));
    lon_mcllcted_nst3(i) = nanmean(lon_cllcted(:,i));
end
for i = 1:n_sz
    prs_length(i) = length(prs{1,i});
end
idx_prs = find(prs_length == max(prs_length));
[mlon_nst3 mprs_nst3] = meshgrid(lon_mcllcted_nst3,prs{1,idx_prs}(:,1)); %arbitrary chosen

%% Bottom topography
lon_for_bathy_nst3 = linspace(min(lon_mcllcted_nst3),max(lon_mcllcted_nst3),100);
lat_for_bathy_nst3 = linspace(max(lat_mcllcted_nst3),min(lat_mcllcted_nst3),100);

for i = 1:length(lon_for_bathy_nst3)
    [BATHY_nst3(i),XGRID_nst3(i),YGRID_nst3(i)] = gebconetcdf('C:\matlib\gebco_2021\gebco_08.nc',lon_for_bathy_nst3(i),lon_for_bathy_nst3(i),lat_for_bathy_nst3(i),lat_for_bathy_nst3(i));
end
...
ax(1) = subplot(2,2,1);
pcolor(mlon_nst3,mprs_nst3,TEMP_cllcted(mprs_nst3(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
[c1 h1] = contour(mlon_nst3,mprs_nst3,TEMP_cllcted(mprs_nst3(1,1):end,:),[0:0.1:1 5:1.5:20],'-w','LabelSpacing',1000);
...
ax(2) = subplot(2,2,2);
pcolor(mlon_nst3,mprs_nst3,sal_cllcted(mprs_nst3(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([34.0 34.5]);
[c1 h1] = contour(mlon_nst3,mprs_nst3,sal_cllcted(mprs_nst3(1,1):end,:),[34:0.1:34.5],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst3,'-r',{'NST3-2','NST3-3','NST3-4','NST3-5'},'LineWidth',.5,'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst3) max(lon_mcllcted_nst3)]);
...
ax(3) = subplot(2,2,3);
pcolor(mlon_nst3,mprs_nst3,dnsty_cllcted(mprs_nst3(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([24.5 27.5]);
[c1 h1] = contour(mlon_nst3,mprs_nst3,dnsty_cllcted(mprs_nst3(1,1):end,:),[24.5:0.25:27.5],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst3,'-r',{'NST3-2','NST3-3','NST3-4','NST3-5'},'LineWidth',.5,'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst3) max(lon_mcllcted_nst3)]);
...
ax(4) = subplot(2,2,4);
pcolor(mlon_nst3,mprs_nst3,fl_cllcted(mprs_nst3(1,1):end,:));shading interp;hold on;
colormap('jet');
set(gca,'YDir','reverse');
caxis([0 0.9]);
[c1 h1] = contour(mlon_nst3,mprs_nst3,fl_cllcted(mprs_nst3(1,1):end,:),[0.2 0.3],'-w','LabelSpacing',1000);
clabel(c1,h1,'color','w');
xline(lon_mcllcted_nst3,'-r',{'NST3-2','NST3-3','NST3-4','NST3-5'},'LineWidth',.5,'LabelVerticalAlignment','bottom', ...
    'LabelHorizontalAlignment','center','FontWeight','b','Fontname','Times New Roman','FontSize',15);
xlim([min(lon_mcllcted_nst3) max(lon_mcllcted_nst3)]);
...
for i = 1:length(files)
    n = n+1;
    ds_NST3_CTD_target{n} = importdata(files(i).name);
end

%% Get maximum size of row of each column
for i = 1:length(files)
    num_of_row_max_NST3(i) = max(size(ds_NST3_CTD_target{1,i}));
end

%% Assign pressure
for i = 1:length(files)
    prs_NST3{i} = ds_NST3_CTD_target{1,i}.data(:,2);
end

%% Creat nan matrix for each variable
prs_new_NST3 = nan(max(num_of_row_max_NST3),length(files));
potemp_new_NST3 = nan(max(num_of_row_max_NST3),length(files));
psal_new_NST3 = nan(max(num_of_row_max_NST3),length(files));

%% Assign rest variables
for i = 1:length(files)
    prs_new_NST3(prs_NST3{1,i},i) = ds_NST3_CTD_target{1,i}.data(:,2);
    potemp_new_NST3(prs_NST3{1,i},i) = ds_NST3_CTD_target{1,i}.data(:,11);
    psal_new_NST3(prs_NST3{1,i},i) = ds_NST3_CTD_target{1,i}.data(:,5);
end

%% Slice some rows to make a consistent form of matrix(otherwise errors hit)
for i = 1:length(files)
    ga_NST3(3:90,i) = sw_gpan(psal_new_NST3(3:90,i),potemp_new_NST3(3:90,i),prs_new_NST3(3:90,i));
end

%% Assign lat, lon from VS, VP CTD data
for i = 1:length(files)
    lat_NST3{i} = ds_NST3_CTD_target{1,i}.data(19,6);
    lon_NST3{i} = ds_NST3_CTD_target{1,i}.data(19,7);
end

%% Convert them into a double form
lat_NST3 = cell2mat(lat_NST3);
lon_NST3 = cell2mat(lon_NST3);

%% Calculate geostrophic velocity
gvel_NST3 = sw_gvel(ga_NST3,lat_NST3,lon_NST3);

%% Level of no motion (lnm)
[m_gvel_NST3 n_gvel_NST3] = size(gvel_NST3);
for i = 1:n_gvel_NST3
    if gvel_NST3(end,i) < 0
        gvel_NST3(4:end,i) = gvel_NST3(4:end,i) + abs(gvel_NST3(m_gvel_NST3,i));
    elseif gvel_NST3(end,i) > 0
        gvel_NST3(4:end,i) = gvel_NST3(4:end,i) - abs(gvel_NST3(m_gvel_NST3,i));
    end
end

%% Find middle points of each lon
for i = 1:length(lon_NST3)-1
    lon_mean_NST3(i) = (lon_NST3(i)+lon_NST3(i+1))./2;
end

%% Define new meshgrids
[mlon_NST3 mdepth_NST3] = meshgrid(lon_mean_NST3,prs_new_NST3(3:90,2));

%% Plot
figure;set(gcf,'position',[403 42 1149 954]);
ax(3) = subplot(2,2,3);
pcolor(mlon_NST3,mdepth_NST3,gvel_NST3(3:end,:));shading interp;hold on;
CT = flip(cbrewer('div','RdBu',50,'linear'),1);
colormap(CT);
set(gca,'YDir','reverse');
caxis([-0.7 0.7001]);
[c1 h1] = contour(mlon_NST3,mdepth_NST3,gvel_NST3(3:end,:),[-0.7:0.1:0.7],'-k','LabelSpacing',1000,'LineWidth',1.5);
